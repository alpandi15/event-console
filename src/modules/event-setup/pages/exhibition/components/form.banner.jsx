...
const FormBanner = () => {
  ...
  const fetchData = useCallback(async () => {
    reset()
    await exhibition.banner.get({ type: mode }).then((res) => {
      res.data.map((v) => {
  ...
  const uploadBanner = async (e, type, index, id) => {
    ...
    formData.append('menu', 'exhibitor')
    if (id) {
      return await exhibition.banner.update(id, formData)
    } else {
      return await exhibition.banner.upload(formData)
    }
  }

  const deleteBanner = async (id) => {
    return await exhibition.banner.delete(id)
  }
  ...
  const saveDescription = async () => {
    ...
    await exhibition.copywriting.update({ title_description: getValues('description') }).then((res) => {
      Toastify({
  ...
  return (
    <div className="p-5">
      ...
      <div className="font-medium text-lg text-slate-700 mt-4">Exhibition Event Banner</div>
      <span>Image banner will appear in exhibition event page  Please upload file with jpg, png extension and size accordingly</span>
