...
const DetailItrexTeam = () => {
  ...
  const fetchData = useCallback(async () => {
    if (!query?.id) return
    setLoading(true)
    await tenant.apiGetDetail(query?.id).then((res) => {
      setData(res?.data)
  ...
  return (
    <>
      <NextSeo title={`Team Tenant | ${data?.first_name ?? ''} ${data?.last_name ?? ''}`} noindex />
      <div className="flex items-center mt-6">
        <h2 className="mr-auto text-2xl font-bold">Detail Team Tenant</h2>
      </div>
        ...
        <div className="col-span-12 lg:!col-span-8">
          <div className="p-5 box mt-5">
            ...
            <div className="mt-4">
              ...
              <div className="font-medium">{data?.nib ?? '-'}</div>
            </div>
            <div className="mt-4">
