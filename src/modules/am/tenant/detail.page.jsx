...
const DetailPage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    await tenantService.apiGetDetail(query?.id).then((res) => {
      setData(res?.data)
    })
    setLoading(false)
  }, [query?.id])
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center mt-16">
        <NextSeo title="Exhibit | Loading..." noindex />
  ...
  return (
    <>
      <NextSeo title={`Tenant | ${data?.first_name ?? ''} ${data?.last_name ?? ''}`} noindex />
      <div className="flex items-center mt-6">
        <h2 className="mr-auto text-2xl font-bold">Detail Tenant F&B</h2>
      </div>
