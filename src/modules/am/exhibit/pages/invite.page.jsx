...
import {apiInvite} from "../sarvices/member.service"
const CreateItrexTeam = () => {
  ...
  const onSubmit = async (values) => {
    ...
    const res = await apiInvite(formData)
    if (!res?.success) {
      Toastify({
        duration: 3000,
        text: res?.message,
  ...
  return (
    <div>
      <NextSeo title="Exhibit | Invite" noindex />
      <div className="flex items-center mt-8">
        <h2 className="mr-auto text-2xl font-medium">Invite Exhibit</h2>
      </div>
