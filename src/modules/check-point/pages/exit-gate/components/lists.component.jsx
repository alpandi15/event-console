...
const ListComponent = ({
  ...
  const items = [
    ...
    },
    {
      ...
      class: 'block w-44',
      render: (row) => (
        <a href={`${publicRuntimeConfig.APPS_HOST}gate/exit/${row.url}`} target='_blank' rel="noreferrer" className='hover:text-blue-500 break-all'>
          {`${publicRuntimeConfig.APPS_HOST}gate/exit/${row.url}`}
        </a>
      )
    },
    {
    ...
    },
    {
      ...
      render: (row) => (
        <div className='flex gap-4'>
          <a href={`/check-point/exit-gate/edit/${row.id}`}>
