...
const CreatePage = () => {
  ...
  const fetchData = async () => {
    if (id) {
      await exit.apiDetail(id).then((res) => {
  ...
  const onSubmit = async (values) => {
    ...
    if (id) {
      await exit.apiUpdate(id, formData).then((res) => {
        Toastify({
          text: 'Success, Exit Gate has been updated',
          type: 'success'
        });
        return router.push('/check-point/exit-gate')
      }).catch((err) => {
        Toastify({
      ...
      await exit.apiCreate(formData).then((res) => {
        Toastify({
          text: 'Success, Exit Gate has been created',
          type: 'success'
        });
        return router.push('/check-point/exit-gate')
      }).catch((err) => {
  ...
  return (
    <div>
      <NextSeo title={`Check Point | Exit Gate | ${id ? 'Edit' : 'Add'}`} noindex />
      <div className="flex items-center mt-8">
        <h2 className="mr-auto text-2xl font-medium">{id ? 'Edit' : 'Add'} Exit Gate</h2>
      </div>
            ...
            <div className="col-span-12 lg:col-span-6">
              ...
              <div className="mt-12 text-right">
                <Console.Button
                  type="button"
                  variant="dark"
                  onClick={() => router.push('/check-point/exit-gate')}
