...
import { apiCreate, apiDetail, apiUpdate } from '../../../services/event'
...
const Form = () => {
    ...
    const saveDraft = async () => {
        ...
        if (id) {
            await apiUpdate(id, formData).then((res) => {
                Toastify({
                    text: 'Success, event survey draft has been saved',
                    type: 'success'
                });
                return router.push('/survey/event-survey')
            }).catch((err) => {
                Toastify({
        ...
        if (questions.length == 0) {
            if (id) {
                ...
                if (id) {
                    await apiUpdate(id, formData).then((res) => {
                        Toastify({
                            text: 'Success update event survey',
                            type: 'success'
                        });
                        return router.push('/survey/event-survey')
                    }).catch((err) => {
                    ...
                    await apiCreate(formData).then((res) => {
                        Toastify({
                            text: 'Success create event survey',
                            type: 'success'
                        });
                        return router.push('/survey/event-survey')
                    }).catch((err) => {
    ...
    return (
        <div>
            <NextSeo title={`Gamification | Event Survey | ${id ? 'Edit' : 'Add'}`} noindex />
            <div className="flex items-center my-8">
                <h2 className="mr-auto text-2xl font-medium">{id ? 'Edit Event Survey' : 'Add Event Survey'}</h2>
            </div>
            <FormProvider {...methods}>
                ...
                <div className="flex flex-col justify-end gap-2 mt-16 md:!flex-row">
                    <Console.Button
                        ...
                        onClick={() => router.push('/survey/event-survey')}
                    >
                        Cancel
                    </Console.Button>
                    <Console.Button
